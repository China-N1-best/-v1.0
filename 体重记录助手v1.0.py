...
def load_weight_data():
    """加载体重数据"""
    try:
        if WEIGHT_DATA_FILE.exists() and WEIGHT_DATA_FILE.stat().st_size > 0:
            df = pd.read_csv(WEIGHT_DATA_FILE)
            # 确保列名正确
            if '日期' not in df.columns or '体重(kg)' not in df.columns:
                st.warning("数据文件格式不正确，将创建新文件")
...
def save_weight_data(df):
    ...
    try:
        if not df.empty:
            # 确保日期格式正确
            df_copy = df.copy()
            df_copy = convert_date_columns(df_copy)
            # 按日期排序
            df_copy = df_copy.sort_values('日期', ascending=True).reset_index(drop=True)
            # 保存到CSV
...
with st.sidebar:
    ...
    with col1:
        if st.button("🗑️ 清空数据", type="secondary", key="clear_data"):
            ...
            if all(col in df.columns for col in required_cols):
                ...
                if not df.empty:
                    ...
                    if not st.session_state.weight_data.empty:
                        ...
                        st.session_state.weight_data = combined
                    else:
                    ...
                    # 保存到文件
                    if save_weight_data(st.session_state.weight_data):
...
if menu == "🏠 主页概览":
    ...
    with col1:
        record_count = len(st.session_state.weight_data)
        st.metric(
            label="总记录天数",
            value=record_count,
            delta=None
    ...
    with col2:
        if record_count >= 2:
            try:
                # 确保数据按日期排序
                sorted_df = st.session_state.weight_data.sort_values('日期')
                first_weight = float(sorted_df['体重(kg)'].iloc[0])
                last_weight = float(sorted_df['体重(kg)'].iloc[-1])
                ...
                st.error(f"计算变化时出错: {e}")
                st.metric("体重变化", "计算错误", delta=None)
        elif record_count == 1:
            try:
                weight = float(st.session_state.weight_data['体重(kg)'].iloc[0])
                st.metric(
                    label="当前体重",
                ...
                st.metric("当前体重", "读取错误", delta=None)
        else:
            st.metric(
                label="体重变化",
                value="0.0kg",
    ...
    with col3:
        if st.session_state.target_info and record_count > 0:
            try:
                current = float(st.session_state.weight_data.sort_values('日期')['体重(kg)'].iloc[-1])
                target = float(st.session_state.target_info.get('target_weight', 0))
                initial = float(st.session_state.target_info.get('current_weight', current))
                
                if current > target and initial > target:
                ...
                st.metric("目标完成度", "计算错误", delta=None)
        elif st.session_state.target_info:
            st.metric("目标设置", "已设置", delta=None)
        else:
            st.metric("目标设置", "未设置", delta=None)
    ...
    if not st.session_state.weight_data.empty:
        ...
        try:
            ...
            # 确保日期是datetime格式
            weight_data_sorted['日期'] = pd.to_datetime(weight_data_sorted['日期'])
...
# 记录体重
    ...
    # 提交按钮
    if st.button("✅ 保存记录", type="primary", use_container_width=True, key="save_record"):
        try:
            # 确保数据格式正确
            new_record_date = pd.Timestamp(record_date)
            ...
            if st.session_state.weight_data.empty:
                ...
                # 确保现有数据的日期列是datetime
                st.session_state.weight_data = convert_date_columns(st.session_state.weight_data)
                
                # 检查是否已有该日期的记录
                existing_dates = st.session_state.weight_data['日期'].tolist()
                
                # 转换为同类型的Timestamp进行比较
                if new_record_date in existing_dates:
                    # 更新已有记录
                    mask = st.session_state.weight_data['日期'] == new_record_date
                    st.session_state.weight_data.loc[mask, '体重(kg)'] = weight
            ...
            if save_weight_data(st.session_state.weight_data):
                ...
                if not st.session_state.weight_data.empty:
                    ...
                    st.info(f"📅 最新记录：{latest_date} - {latest_weight:.1f}kg")
            else:
    ...
    if not st.session_state.weight_data.empty:
        ...
        try:
            ...
            display_df = convert_date_columns(display_df)
            display_df = display_df.sort_values('日期').reset_index(drop=True)
            ...
            # 确保体重是数值类型
            display_df_display['体重(kg)'] = pd.to_numeric(display_df_display['体重(kg)'], errors='coerce')
            
            # 计算变化
            display_df_display['变化'] = display_df_display['体重(kg)'].diff()
            display_df_display['变化%'] = (display_df_display['体重(kg)'].pct_change() * 100).round(2)
            ...
            display_df_display['体重(kg)'] = display_df_display['体重(kg)'].apply(format_weight)
                    ...
                    with col_del2:
                        if st.button("确认删除", type="secondary", key="confirm_delete"):
                            try:
                                ...
                                # 确保现有数据日期格式正确
                                current_data = convert_date_columns(st.session_state.weight_data)
                                
                                # 删除选中的记录
                                mask = ~current_data['日期'].isin(dates_to_delete_dt)
                                st.session_state.weight_data = current_data[mask].reset_index(drop=True)
...
# 设置目标
    ...
    if not st.session_state.weight_data.empty:
        try:
            # 确保数据按日期排序
            sorted_df = st.session_state.weight_data.sort_values('日期')
            current_weight = float(sorted_df['体重(kg)'].iloc[-1])
            st.info(f"📊 当前最新体重：**{current_weight:.1f}kg**")
        except Exception as e:
            st.warning(f"获取当前体重时出错: {e}")
        ...
        current_weight = st.number_input(
            ...
            value=70.0,
            step=0.1,
            key="current_weight_input_no_data"
        )
    ...
    if current_weight is not None:
        ...
        if weight_to_lose > 0:
            ...
            st.warning(f"目标体重 ({target_weight}kg) 低于当前体重 ({current_weight:.1f}kg)，您可能需要增重哦！")
        else:
            st.info("您已经达到目标体重！")
    
    # 保存目标
    if st.button("💾 保存目标计划", type="primary", use_container_width=True, key="save_target"):
...
# 图表分析
    ...
    if st.session_state.weight_data.empty:
        ...
        try:
            ...
            weight_data = convert_date_columns(weight_data)
            weight_data = weight_data.sort_values('日期')
            ...
            with tab1:
                ...
                # 确保体重是数值类型
                weight_data['体重(kg)'] = pd.to_numeric(weight_data['体重(kg)'], errors='coerce')
            ...
            with tab2:
                # 确保体重是数值类型
                if '体重(kg)' in weight_data.columns:
                    weight_data['体重(kg)'] = pd.to_numeric(weight_data['体重(kg)'], errors='coerce')
                ...
                if len(weight_data) > 1:
                    fig2.add_trace(
                        go.Bar(
                            ...
                            name='日变化',
                            marker_color='#FFD166',
                            hovertemplate='<b>%{x|%Y-%m-%d}</b><br>变化: %{y:+.1f}kg<extra></extra>'
                        ),
                        row=2, col=1
                    )
            ...
            with tab3:
                ...
                with col_stat1:
                    ...
                    stats = {}
                    try:
                        if not weight_data.empty and '体重(kg)' in weight_data.columns:
                            weight_values = pd.to_numeric(weight_data['体重(kg)'], errors='coerce')
                            weight_values = weight_values.dropna()
                            
                            if not weight_values.empty:
                                stats['平均体重'] = f"{weight_values.mean():.2f}kg"
                                stats['最高体重'] = f"{weight_values.max():.2f}kg"
                                stats['最低体重'] = f"{weight_values.min():.2f}kg"
                                stats['体重范围'] = f"{weight_values.max() - weight_values.min():.2f}kg"
                                stats['记录天数'] = f"{len(weight_values)} 天"
                                
                                if len(weight_data) > 1:
                                    time_span = (weight_data['日期'].iloc[-1] - weight_data['日期'].iloc[0]).days + 1
                                    stats['时间跨度'] = f"{time_span} 天"
                                
                                if len(weight_values) > 1:
                                    changes = weight_values.diff().dropna()
                                    if len(changes) > 0:
                                        stats['总变化'] = f"{weight_values.iloc[-1] - weight_values.iloc[0]:+.2f}kg"
                                        stats['平均日变化'] = f"{changes.mean():+.3f}kg/天"
                                        stats['最大单日变化'] = f"{changes.max():+.2f}kg"
                                        stats['最小单日变化'] = f"{changes.min():+.2f}kg"
                    except Exception as e:
                ...
                with col_stat2:
                    ...
                    if len(weight_data) > 1:
                        ...
                        if len(recent_data) > 1:
                            try:
                                recent_weights = pd.to_numeric(recent_data['体重(kg)'], errors='coerce')
                                recent_weights = recent_weights.dropna()
                                
                                if len(recent_weights) > 1:
                                    recent_change = recent_weights.iloc[-1] - recent_weights.iloc[0]
                                    avg_daily_change = recent_change / (len(recent_weights) - 1)
    ...
    if st.session_state.weight_data.empty:
        ...
        try:
            ...
            if not weight_data.empty:
                ...
                current_weight = float(target_info.get('current_weight', 0))
            ...
            with col_overview2:
                if daily_target > 0 and remaining > 0:
            ...
            with col_overview3:
                if daily_target > 0 and remaining > 0:
                    days_left = int(remaining / daily_target)
            ...
            fig.update_layout(
                title=f"体重预测 ({show_days if not weight_data.empty else 0}天历史 + {prediction_days}天预测)",
                xaxis_title="日期",
                yaxis_title="体重 (kg)",
                template='plotly_white',
                height=500,
            ...
            # 分页显示预测
            page_size = st.selectbox("每页显示天数", [7, 14, 30], index=1, key="page_size_select")
